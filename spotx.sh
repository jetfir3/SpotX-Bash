...
if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then
  BD86="6JGduAjMy0yNzYTZjNjYycmL"
  BD64="6JGduEjMy0yNzYTZjNjYycmL"
...
if [[ "${PLATFORM_TYPE}" == "macOS" ]]; then if [[ "${NOTINSTALLED_VAR}" == 'false' ]]; then if [ -z ${FORCE_VERSION_FLAG+x} ]; then CLIENT_VERSION=$(awk '/CFBundleShortVersionString/{getline; print}' "${INSTALL_PATH}/Spotify.app/Contents/Info.plist" 2>/dev/null | cut -d\> -f2- | rev | cut -d. -f2- | rev); fi; fi; fi
if [[ "${PLATFORM_TYPE}" == "Linux" ]]; then if [ -z ${FORCE_VERSION_FLAG+x} ]; then CLIENT_VERSION=$("${INSTALL_PATH}"/spotify --version | cut -d " " -f3- | rev | cut -d. -f2- | rev); fi; fi
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  if [[ "${EXCLUDE_EXPERIMENTAL_FLAG}" == "true" ]]; then
    ...
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.6.861") ]]; then $PERL "${ENABLE_ADD_PLAYLIST}" "${XPUI_JS}"; fi
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.99.871") ]]; then $PERL "${ENABLE_ATTACK_ON_TITAN_EASTER_EGG}" "${XPUI_JS}"; fi
    ...
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.7.1264") ]]; then $PERL "${ENABLE_CONCERTS_INTERESTED}" "${XPUI_JS}"; fi
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.7.1264") ]]; then $PERL "${ENABLE_CULTURAL_MOMENTS}" "${XPUI_JS}"; fi
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.99.871") && $(ver "${CLIENT_VERSION}") -lt $(ver "1.2.6.861") ]]; then $PERL "${ENABLE_DEVICE_LIST_LOCAL}" "${XPUI_JS}"; fi
    ...
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.0.1155") && $(ver "${CLIENT_VERSION}") -lt $(ver "1.2.6.861") ]]; then $PERL "${ENABLE_FORGET_DEVICES}" "${XPUI_JS}"; fi
    ...
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.5.954") ]]; then $PERL "${ENABLE_YOUR_DJ}" "${XPUI_JS}"; fi
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  if [[ "${OLD_UI_FLAG}" == "false" && $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.93.896") && "${EXCLUDE_EXPERIMENTAL_FLAG}" == "false" ]]; then
    ...
    if [[ "${DISABLERIGHTSIDEBAR_OPT}" == "true" ]]; then perl -e 'print "\xE2\x9C\x94\x20\x44\x69\x73\x61\x62\x6C\x65\x64\x20\x72\x69\x67\x68\x74\x20\x73\x69\x64\x65\x62\x61\x72\n"'; else if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.1.98.683") ]]; then $PERL "${ENABLE_RIGHT_SIDEBAR}" "${XPUI_JS}"; if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.7.1264") ]]; then $PERL "${ENABLE_RIGHT_SIDEBAR_CREDITS}" "${XPUI_JS}"; $PERL "${ENABLE_RIGHT_SIDEBAR_MERCH_FALLBACK}" "${XPUI_JS}"; fi; fi; fi
    ...
    if [[ "${DISABLESIDEBARLYRICS_OPT}" == "true" ]]; then perl -e 'print "\xE2\x9C\x94\x20\x44\x69\x73\x61\x62\x6C\x65\x64\x20\x73\x69\x64\x65\x62\x61\x72\x20\x6C\x79\x72\x69\x63\x73\n"'; else if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.0.1165") ]]; then $PERL "${ENABLE_RIGHT_SIDEBAR_LYRICS}" "${XPUI_JS}"; fi
    if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.7.1264") ]]; then $PERL "${ENABLE_PANEL_SIZE_COORDINATION}" "${XPUI_JS}"; fi; fi
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.1.958") ]]; then $PERL "${ENABLE_HPTO_LOCATION_REFACTOR}" "${XPUI_JS}"; fi
  if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.3.1107") ]]; then $PERL "${ENABLE_USER_FRAUD_VERIFICATION}" "${XPUI_JS}"; fi
  if [[ $(ver "${CLIENT_VERSION}") -ge $(ver "1.2.5.954") ]]; then $PERL "${ENABLE_USER_FRAUD_VERIFICATION_REQUEST}" "${XPUI_JS}"; fi; fi
